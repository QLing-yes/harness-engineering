# 跨文章深层洞见：7 篇文章 + 实践记录交叉对比

> 对 references/ 中 7 篇文章、concepts/ 6 大概念、practice/ Ralph 实验的交叉分析
> 日期：2026-04-09
...
| 学派 | 代表 | 核心主张 | 瓶颈在哪 |
|------|------|---------|---------|
| **约束派** | OpenAI、Fowler、HumanLayer | 答案是更好的约束 | 解空间太大，模型不可靠 |
...
这三个学派**都有数据支撑**，但目前 repo 中的概念体系偏向约束派（来自 OpenAI 原文），对架构派和怀疑派的整合不够。

**我的判断：** 三者不是互斥的，而是作用在不同层次上：
- 约束派解决**当前模型的可靠性**问题（战术层）
- 架构派解决**跨模型代际的可维护性**问题（战略层）
- 怀疑派解决**整个范式是否值得投入**的问题（元层）

一个成熟的 harness engineering 实践应该同时容纳三个视角。
...
**这意味着：** Harness Engineering 目前只能在**需求明确、可测试**的领域可靠工作。对于需求模糊、正确性难以形式化的领域（设计、产品决策、安全审计），harness 的背压机制无法提供足够保障。Anthropic #4 的 Evaluator 是目前最接近解决这个问题的尝试，但它自己也承认 self-evaluation 会失败。
...
3. "人类掌舵"在 meta-harness 时代具体意味着什么？
4. 评估问题的可扩展解法是什么？形式化验证？对抗性测试？人类抽检？
